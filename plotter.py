...
if __name__ == '__main__':
    ...
    if args.fig == 3:
        ...
        fig.text(0.03, 0.92, 'A', size=28, weight='bold')
        fig.text(0.5, 0.92, 'B', size=28, weight='bold')
        legend_elements = [
            Patch(facecolor=get_color('nn'), edgecolor='black', linewidth=1.2, label='NN'),
            Patch(facecolor=get_color('rf'), edgecolor='black', linewidth=1.2, label='RF'),
            Patch(facecolor=get_color('gpc'), edgecolor='black', linewidth=1.2, label='GPC'),
            Patch(facecolor=get_color('gpr'), edgecolor='black', linewidth=1.2, label='GPR'),
            Patch(facecolor=get_color('sv'), edgecolor='black', linewidth=1.2, label='SV'),
            Patch(facecolor=get_color('xgb'), edgecolor='black', linewidth=1.2, label='XGB'),
        ]
        fig.legend(
    ...
    if args.fig == 6:
        ...
        plt.rcParams['legend.fontsize'] = 16
        plt.rcParams['figure.titlesize'] = 12
        error_kw = {'capsize': 2.0, 'elinewidth': 1.1, 'capthick': 1.1}
        sampler_colors = {
            'maximin': '#fe7c7c',
            'medoids': '#91dbff',
            'max_entropy': '#01ce90',
        ...
        for sampler, data in plot_data.items():
            axs[0].scatter(rounds, data[:,0], s=100.0, label=sampler_labels[sampler], color=sampler_colors[sampler], edgecolor='black', linewidth=1.0, zorder=4, clip_on=False)
        ...
        axs[0].plot([-1.0, 11.0], [1.0, 1.0], color='black', linestyle='dashed', zorder=1, linewidth=1.75)
        axs[0].tick_params(axis='both', top=True, left=True, right=True, bottom=True, direction='in', width=1.75, length=4.0)
        ...
        axs[0].set_title('Active Learning', fontsize=22)
        axs[0].legend(fancybox=False, loc='upper right', edgecolor='white')
        axs[0].text(-0.1, 1.05, 'A', transform=axs[0].transAxes, size=28, weight='bold')

        # Make space-filling plot.
        plot_data = pickle.load(open('data/fig6_sf.pkl', 'rb'))
        for sampler, data in plot_data.items():
            axs[1].scatter(rounds, data[:,0], s=100.0, label=sampler_labels[sampler], color=sampler_colors[sampler], edgecolor='black', linewidth=1.0, zorder=4, clip_on=False)
        ...
        axs[1].plot([-1.0, 11.0], [1.0, 1.0], color='black', linestyle='dashed', zorder=1, linewidth=1.75)
        axs[1].tick_params(axis='both', top=True, left=True, right=True, bottom=True, direction='in', width=1.75, length=4.0)
        axs[1].set_xlabel('Rounds of Active Learning')
        axs[1].set_xlim(xmin=-1.0, xmax=11.0)
        axs[1].set_title('Space-Filling', fontsize=22)
        axs[1].text(-0.1, 1.05, 'B', transform=axs[1].transAxes, size=28, weight='bold')
    ...
    if args.fig == 7:
        ...
        error_kw = {'ecolor': 'black', 'capsize': 2.0, 'elinewidth': 1.1, 'capthick': 1.1}
        fig, axs = plt.subplots(1, 3, figsize=(13,5.1))
        ...
        fig.text(0.03, 0.90, 'A', size=34, weight='bold')
        fig.text(0.35, 0.90, 'B', size=34, weight='bold')
        fig.text(0.68, 0.90, 'C', size=34, weight='bold')
        legend_elements = [
            Patch(facecolor=get_colors('NN', ensemble=False), edgecolor='black', linewidth=1.2, label='NN'),
            Patch(facecolor=get_colors('RF', ensemble=False), edgecolor='black', linewidth=1.2, label='RF'),
            Patch(facecolor=get_colors('Ensemble', ensemble=False), edgecolor='black', linewidth=1.2, label='Ensemble'),
        ]
        fig.legend(
            handles=legend_elements, 
            loc='lower center', 
    ...
    if args.fig == 9:
        ...
        plt.rcParams['figure.titlesize'] = 12
        error_kw = {'ecolor': 'black', 'capsize': 2.0, 'elinewidth': 1.1, 'capthick': 1.1}
        fig, axs = plt.subplots(1,2,figsize=(14,5), width_ratios=[1, 1.5])
        ...
        for model, data in final_data.items():
            if '_ard' in model:
                ...
                marker = 'o'
            axs[0].scatter(data[:,0], data[:,1], s=100.0, marker=marker, color=get_color(model), edgecolor='black', linewidth=1.0, zorder=3, clip_on=False, label=get_label(model))
            axs[0].plot(data[:,0], data[:,1], color=get_color(model), alpha=0.5, zorder=2)
        ...
        axs[0].legend(loc='upper left', fancybox=False, edgecolor='black', ncol=3, handletextpad=0.3, columnspacing=0.5)
        axs[0].text(-0.2, 1.00, 'A', transform=axs[0].transAxes, size=34, weight='bold')
        ...
        plt.rcParams['legend.fontsize'] = 16
        plt.rcParams['figure.titlesize'] = 12
        error_kw = {'capsize': 3.0, 'elinewidth': 1.1, 'capthick': 1.1}
        ...
        axs[1].tick_params(axis='both', direction='in', left=True, bottom=False, right=True, top=False, width=2.0, length=5.0)
        axs[1].set_ylim(ymin=0.0, ymax=250.0)
        axs[1].tick_params(axis='y', labelcolor=ns_ratio_font_color)
        axs[1].tick_params(axis='x', pad=7)
        axs[1].text(-0.2, 1.00, 'B', transform=axs[1].transAxes, size=34, weight='bold')
        ...
        ax2.tick_params(axis='y', direction='in', right=True, width=2.0, length=5.0)
